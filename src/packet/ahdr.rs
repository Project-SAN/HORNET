...
mod tests {
    ...
    fn ahdr_expiry_check() {
        ...
        let rmax = 1usize;
        let mut svb = [0u8; 16]; rng.fill_bytes(&mut svb); let sv = Sv(svb);
        let mut key = [0u8; 16]; rng.fill_bytes(&mut key); let si = Si(key);
        let rseg = RoutingSegment(alloc::vec![0u8; 8]); let exp = Exp(1_234_567);
        ...
        let err = proc_ahdr(&sv, &ahdr, Exp(exp.0)).err().expect("must error at exp");
