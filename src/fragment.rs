...
fn read_be_u32(b: &[u8]) -> u32 { let mut t=[0u8;4]; t.copy_from_slice(b); u32::from_be_bytes(t) }

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct FragHeader { pub msg_id: u32, pub index: u16, pub total: u16 }
...
impl FragHeader {
    ...
    pub fn decode(buf: &[u8]) -> Result<(Self, &[u8])> {
        if buf.len() < HDR_LEN { return Err(Error::Length); }
        let msg_id = read_be_u32(&buf[0..4]);
        let index = read_be_u16(&buf[4..6]);
        let total = read_be_u16(&buf[6..8]);
        if total == 0 || index as u32 >= total as u32 { return Err(Error::Length); }
        Ok((FragHeader { msg_id, index, total }, &buf[HDR_LEN..]))
    }
}
...
pub fn split(msg: &[u8], cap: usize, msg_id: u32) -> Result<Vec<Vec<u8>>> {
    if cap <= HDR_LEN { return Err(Error::Length); }
    ...
    for i in 0..(total as usize) {
        ...
        let hdr = FragHeader { msg_id, index: i as u16, total };
...
struct Entry { msg_id: u32, total: u16, chunks: alloc::vec::Vec<Option<Vec<u8>>> }

impl Reassembler {
    pub fn new(capacity: usize) -> Self { Self { entries: Vec::new(), capacity } }
    ...
    pub fn accept(&mut self, frag: &[u8]) -> Result<Option<Vec<u8>>> {
        ...
        for (i, e) in self.entries.iter().enumerate() {
            if e.msg_id == hdr.msg_id { idx = Some(i); break; }
        }
        if idx.is_none() {
            ...
            self.entries.push(Entry { msg_id: hdr.msg_id, total: hdr.total, chunks });
            idx = Some(self.entries.len() - 1);
        }
        let e = &mut self.entries[idx.unwrap()];
        if e.total != hdr.total { return Err(Error::Length); }
        let pos = hdr.index as usize;
        if pos >= e.chunks.len() { return Err(Error::Length); }
        if e.chunks[pos].is_none() { e.chunks[pos] = Some(Vec::from(body)); }
        // check completion
        if e.chunks.iter().all(|c| c.is_some()) {
            let mut merged = Vec::new();
            for c in e.chunks.iter_mut() { merged.extend_from_slice(c.as_ref().unwrap()); }
            // remove entry
...
mod tests {
    ...
    fn split_and_reassemble_in_order() {
        ...
        let mut out = None;
        for f in frags.iter() { out = r.accept(f).unwrap(); }
    ...
    fn split_and_reassemble_out_of_order() {
        ...
        let mut got = None;
        for f in frags.iter() { let x = r.accept(f).unwrap(); if x.is_some() { got = x; } }
