...
fn main() {
    use hornet::{
        packet::{ahdr, fs_core, fs_payload, onion},
        sphinx,
        sphinx::strict,
        types::{C_BLOCK, Exp, RoutingSegment, Sv},
    };
    ...
    fn gen_node(seed: u64) -> ([u8; 32], [u8; 32], Sv) {
        ...
        let mut svb = [0u8; 16];
        XorShift64(seed ^ 0x55aa_aa55_dead_beef)
    ...
    let rs_b: Vec<RoutingSegment> = (0..lb)
        .map(|i| RoutingSegment(vec![0x80 | (i as u8); 8]))
        .collect();
    ...
    for i in (0..lf).rev() {
        ...
        for (b, m) in sp_f.iter_mut().zip(mask.iter()) {
            *b ^= *m;
        }
    }
    ...
    for i in 0..lf {
        ...
        let si_node =
            strict::node_process_forward_strict(&mut shdr_f, &sk_i).expect("strict forward hop");
    ...
    let (mut shdr_b, keys_b, _eph_pub_b) =
        strict::source_create_forward_strict(&x_s, &pubs_b, beta_len);
    ...
    for j in 0..lb {
        ...
        let si_j = strict::node_process_forward_strict(&mut shdr_b, &sk_j)
            .expect("strict backward hop header");
        // Add SPb layer with PRG1(Si)
        let mut mask = vec![0u8; sp_b.0.len()];
        hornet::crypto::prg::prg1(&si_j.0, &mut mask);
        for (b, m) in sp_b.0.iter_mut().zip(mask.iter()) {
            *b ^= *m;
        }
    ...
    let pf_recv = fs_payload::FsPayload {
        bytes: pf_bytes.clone(),
        rmax,
    };
    let fses_f =
        fs_payload::retrieve_fses(&keys_f, &seed_f, &pf_recv).expect("retrieve fs forward");
    let fses_b = fs_payload::retrieve_fses(&keys_b, &seed_b, &p_b).expect("retrieve fs backward");
    assert!(
        fses_f.len() == fses_f_vec.len()
            && fses_f
                .iter()
                .zip(fses_f_vec.iter())
                .all(|(a, b)| a.0 == b.0),
        "Forward FS list round-trips"
    );
    assert!(
        fses_b.len() == fses_b_vec.len()
            && fses_b
                .iter()
                .zip(fses_b_vec.iter())
                .all(|(a, b)| a.0 == b.0),
        "Backward FS list round-trips"
    );
    ...
    for k in (0..lb).rev() {
        onion::remove_layer(&keys_b_rev[k], &mut iv2, &mut data2).expect("remove layer");
    }
